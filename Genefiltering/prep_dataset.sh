...
cd ..
ln -sr Genefiltering/results/assemblies/* data/CDS_filtered
cp -r Genefiltering/results/assemblies CUB_analysis

cd CUB_analysis/assemblies
for file in *faa; do mkdir ${file%.faa} && mv $file ${file%.faa}; done  # create separate directories for each genome 
echo "please copy your ribosomal fasta files of each <species> to the CUB_analysis/assemblies/<species> directory before continuing with the next step"
