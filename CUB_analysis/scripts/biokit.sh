...
for file in filtered*faa; do biokit rscu $file > ${file%.faa}.rscu; done 

#gene wise relative synonymous codon usage 
for file in filtered*faa; do biokit grscu $file > ${file%.faa}.grscu; done 

#gc content per codon position
biokit gc_content_first_position filtered*faa -v > gc1
biokit gc_content_second_position filtered*faa -v > gc2
biokit gc_content_third_position filtered*faa -v > gc3


# translate to AA 
for file in filtered*faa; do cp  $file ${file%.faa}fasta; done 
